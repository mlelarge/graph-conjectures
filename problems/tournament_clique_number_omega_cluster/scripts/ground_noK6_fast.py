...
t=time.time()
try:
    chains = enumerate_transitive_sets(6, 500)
    print(f"transitive 6-sets: {len(chains)}  ({time.time()-t:.1f}s)"); sys.stdout.flush()
except TimeoutError:
    print("ENUMERATION TIMEOUT at 500s -> no-K6 SAT INFEASIBLE at order 63"); sys.exit(0)
...
with Cadical153(bootstrap_with=cnf.clauses) as m:
    sat=m.solve()
print(f"no-K6 SAT={sat} ({time.time()-t:.1f}s)  => omega_vec>=6 is {not sat}")
print("RESULT: omega_vec(C3[AC7[C3]]) >= 6:", not sat)
