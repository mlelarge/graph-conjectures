"""DECISIVE p=19 probe: is the almost-consecutive circulant
    AC_19 = Cay(Z/19, g={1..8} U {10})
3-omega_vec-critical?  (the rule g(p)={1..(p-3)/2} U {(p+1)/2})

Reuses the SAME verified triangle-free lower-bound method as reverify_ac17.py
...
P = 19
g = set(range(1, 9)) | {10}   # {1..8} U {10} = {1..(p-3)/2} U {(p+1)/2}
...
# generator validity: g and -g partition {1..18}
neg = {(-d) % P for d in g}
out["g_partition_ok"] = (sorted(g | neg) == list(range(1, P)) and (g & neg) == set())
out["abs_g_size_half"] = (len(g) == (P - 1) // 2)
print("g       =", sorted(g))
print("-g mod19=", sorted(neg))
print("partition {1..18}:", out["g_partition_ok"], "|g|=(p-1)/2:", out["abs_g_size_half"])
...
print("19 mod 4 =", P % 4, "consecutive:", out["consecutive_generators"])

# ---- the verified triangle-free decision (copied verbatim from reverify_ac17) ----
...
print("\nUPPER BOUND min omega over 19 rotations:", best_w)
...
print("=> omega_vec(AC_19) =", omega_vec)

# ---- DELETION (canonical, vertex-transitive => all 19 equal) ----
...
print("deletion omega_vec(AC_19 - vertex 0):", del_ov, f"({out['deletion_time_s']}s)")

out["is_3_critical"] = (omega_vec == 3 and del_ov == 2)
print("\n3-omega_vec-critical:", out["is_3_critical"])

with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'data', 'probe_p19.json'), "w") as f:
