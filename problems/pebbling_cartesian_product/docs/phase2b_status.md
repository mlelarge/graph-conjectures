...
## FPY format diagnostics — current-source probe (2026-05-09 update)

Running their classes live (`scripts/fpy_probe.py`, `gurobipy` stubbed
out) against the source in `external/Graph_Pebbling/py/` exposes the
runtime semantics directly:

- **Vertex labels** are strings like `"('0', '1')"` (FPY calls
  `str()` on cartesian-product tuples whose entries are themselves
  strings, so the inner single quotes survive). This is what makes
  `int(v[2])` and `int(v[7])` line up: positions 2 and 7 land on the
  digits.
- **Root** is the same string format when used via the live `main()`
  loop (line 571: `for r in initPebblingGraph.nodes`).
- **`saveCertificate` matrix convention is `cert[i][j] = w((i, j))`,
  no transpose** — confirmed by hand-building a strategy with
  `w((0,0))=2, w((1,1))=2` and reading the file back.
- **`saveEdges` writes** rows `i, "('a', 'b')", "('c', 'd')"` (NetworkX
  source-target columns).

The earlier diagnostic block below (which concluded "matrix is
transposed, edges file has fan-in 2") was **derived from the published
`ls-bounds-updated/` CSVs**, which the original analysis already
flagged as having been generated by a code path different from the
current `main` branch. Trusting the published artefacts and
trusting the current source code give contradictory answers; we trust
the current source.

The adapter (`scripts/ingest_flocco_pulaj_yerger.py`) had a real bug
that only the live probe caught: `_parse_pair` didn't strip the inner
single quotes. Fixed; covered by
`tests/test_phase2b_fpy_ingest.py::test_parse_pair_accepts_live_fpy_form`
and `::test_parse_edges_csv_live_fpy_form`. End-to-end round-trip
(synthetic strategy → `TreeStrategy.saveCertificate` →
`parse_weight_csv` / `parse_edges_csv`) is byte-stable: weights and
edges round-trip exactly.

What this does **not** unblock: re-checking the published
`ls-bounds-updated/` CSVs, which were produced by a different code
path. To rationally check FPY 96, we need either a fresh MILP run
under current code (Gurobi licence required), or a structured (JSON /
pickle) dump from the authors.

## FPY format diagnostics — published-CSV reading (earlier)
