...
$\dminus(\Ln) \ge 17/16 + 1/4$ uniformly across $n \in [4, 1000]$ with
slack $\sim 0.257$, and the propagated forward-error bound on
$\dminus$ is $\le 1.42 \cdot 10^{-4}$.
...
constant for the exact \texttt{dsyevd} path; rather, we adopt the
\emph{conservative working envelope} $c \le 10^{4}$ (which is
overshooting any published constant by at least one order of
magnitude) and check below that the resulting forward-error bound is
dwarfed by the empirical slack by more than three orders of
magnitude. The role of this step is therefore to bound the
floating-point error by a number that is provably safe under any
plausible refinement of the constant. Taking $C_{n} \le 10^{4}\, n$
...
envelope $c \le 10^{4}$ in the Demmel--Kahan-style bound is conservative
and consistent with published constants for backward-stable symmetric
eigensolvers, but is not formally derived for the exact LAPACK
\texttt{dsyevd} pipeline within this paper; cf.\
...
plausible sharper constant yields the same conclusion. A confirmatory
